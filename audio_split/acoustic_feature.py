...
class VAD:
    ...
    def __init__(self, wav_file, frame_len=400, min_interval=15, e_low_multifactor=1.0, zcr_multifactor=1.0, pt=True,
        ...
        for d in self.wav_dat_split_f:
            res = d[1] - d[0]
            time = res * self.frame_len_time
            self.wav_dat_split_t.append(time)

        i = 0
        while i < len(self.wav_dat_split_t):
            if self.wav_dat_split_t[i] <= 500:
                self.wav_dat_split_f.pop(i)
                self.wav_dat_split_t.pop(i)
            else:
                i += 1
        i = 0
        while i < len(self.wav_dat_split_t):
            if i != 0 and self.wav_dat_split_f[i][0] - self.wav_dat_split_f[i - 1][1] <= 20:
                self.wav_dat_split_f.pop(i)
                self.wav_dat_split_t.pop(i)
            else:
                i += 1
